...
export default function AddEmployee({ navigation }) {
  ...
  const laundryId = route?.params?.id || '';     // who is creating
  ...
  const onAddEmployee = async () => {
    ...
    const payload = {
      ...
      email    : email.trim(),
      role     : role.value,
      laundryId,                 // include if your backend needs it
    };
    ...
    try {
      ...
      navigation.navigate('Employees', { token, id: laundryId, refresh: Date.now() });
    } catch (err) {
