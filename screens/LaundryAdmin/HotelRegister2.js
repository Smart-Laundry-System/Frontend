...
// ✅ added: axios + API helpers
import axios from 'axios';
import { API_URL, uploadImageFile } from '../../Services/api'; // adjust path if this file is elsewhere
...
function HotelRegister2({ navigation }) {
    ...
    // ❌ removed uploadImageToBackend (old)
    // ✅ handleRegister now uses uploadImageFile
    const handleRegister = async () => {
        try {
            // upload single "Others(Upload an image)" if present
            let uploadedOthersUrl = null;
            if (selectedImage) {
                const { absoluteUrl } = await uploadImageFile(selectedImage);
                uploadedOthersUrl = absoluteUrl; // e.g. http://172.20.10.3:3999/files/xxxx.png
            }

            // (optional) upload multiple laundry images if you want to persist them too
            // If you don't need this yet, you can skip this block.
            // Keeping it minimal: only include local previews; no server upload loop here.
            const userData = {
                name: "Hotel ABC",
                ...
                phone: "0771234567",
                role: "LAUNDRY",
                image: uploadedOthersUrl, // single image absolute URL (or null)
                cloths: selectedCloths,
                types: selectedTypes
            };

            const response = await axios.post(`${API_URL}/auth/v1/addLaundry`, userData);
            ...
            Alert.alert("Upload/Registration error", error.message || "Unknown error");
        }
    };
                            ...
                            <View style={[styles.fields, {
                                ...
                                {images.length > 0 ? (
                                    images.map((image, index) => (
                                        <View style={{alignItems: 'center'}} key={`img-${index}`}>
                                            <Image
